...
internal class FlagShaderGUI : ShaderGUI
{
    ...
    private static class Styles
    {
        ...
        public static GUIContent useSmoothText = EditorGUIUtility.TrTextContent("useSmooth", " Ture:Metallic (A) andSmoothness (A)");
        public static GUIContent smoothnessText = EditorGUIUtility.TrTextContent("Smooth Map", "RGB (R)");
        ...
        public static GUIContent DayNightText = EditorGUIUtility.TrTextContent("Day Night", "Day:1;Night:0 (R)");
        public static GUIContent EmissionStrengthText = EditorGUIUtility.TrTextContent("EmissionStrength", "Emission Strength(R) and Day or Night Mask(A)");
    ...
    MaterialProperty useSmooothTexture = null;
    ...
    MaterialProperty EmissionStrengthMap = null;

    MaterialProperty DayNight = null;
    MaterialProperty IsDayTime = null;
    MaterialProperty tilingOffset = null;

    MaterialProperty Frequency = null;

    MaterialProperty AmplitudeStrength = null;

    MaterialProperty InvWaveLength = null;
    MaterialProperty Fold = null;
    MaterialProperty TextureRow = null;
    MaterialProperty localU = null;
    ...
    public void FindProperties(MaterialProperty[] props)
    {
        ...
        useSmooothTexture = FindProperty("useSmoothTexture", props);
        smoothnessMap = FindProperty("smoothnessTexture", props);
        ...
        EmissionStrengthMap = FindProperty("EmissionStrengthTexture", props);
        //DayNight = FindProperty("dayNightTexture", props);
        IsDayTime = FindProperty("_IsDayTime", props);
        ...
        AmplitudeStrength = FindProperty("_AmplitudeStrength", props);
        InvWaveLength = FindProperty("_InvWaveLength", props);
        TextureRow = FindProperty("_TextureRow", props);
        Fold = FindProperty("_Fold", props);
        localU = FindProperty("_LocalU", props);
    }
    ...
    public void ShaderPropertiesGUI(Material material)
    {
        ...
        EditorGUI.BeginChangeCheck();
        {
            ...
            //DoDayNightArea(material);
            m_MaterialEditor.ShaderProperty(IsDayTime, "IsDayTime");
            //EditorGUI.BeginChangeCheck();
        ...
        m_MaterialEditor.ShaderProperty(Frequency, "波动频率");
        m_MaterialEditor.ShaderProperty(AmplitudeStrength, "振幅强度");
        m_MaterialEditor.ShaderProperty(InvWaveLength, "波长的倒数");
        m_MaterialEditor.ShaderProperty(Fold, "褶皱程度");
        m_MaterialEditor.ShaderProperty(TextureRow, "UV列数");
        m_MaterialEditor.ShaderProperty(localU, "锁定垂直方向");
        if (EditorGUI.EndChangeCheck())
        {
    ...
    void DoEmissionArea(Material material)
    {
        ...
        // }
        m_MaterialEditor.TexturePropertySingleLine(Styles.EmissionStrengthText,EmissionStrengthMap);
    }

    void DoDayNightArea(Material material)
    {
        m_MaterialEditor.TexturePropertySingleLine(Styles.DayNightText, DayNight);
    }
    ...
    void DoSpecularMetallicArea(Material material)
    {
        ...
        m_MaterialEditor.ShaderProperty(useSmooothTexture, "useSmooth");

        bool showSmoothTexture = material.GetFloat("useSmoothTexture") == 1;
        if (showSmoothTexture)
        {
            m_MaterialEditor.TexturePropertySingleLine(Styles.smoothnessText, smoothnessMap);
        }

        int indentation = 2; // align with labels of texture properties
        m_MaterialEditor.ShaderProperty(showSmoothnessScale ? smoothnessScale : smoothnessMap, showSmoothnessScale ? Styles.smoothnessScaleText : Styles.smoothnessText, indentation);
    ...
    static void SetMaterialKeywords(Material material)
    {
        ...
        SetKeyword(material, "LOCALUVALUE", material.GetFloat("_LocalU") == 1);
        SetKeyword(material, "METALLICGLOSSTEXTURE", material.GetTexture("metallicGlossTexture"));
        SetKeyword(material, "_SMOOTHTEXTURE_", material.GetFloat("useSmoothTexture") == 1);
